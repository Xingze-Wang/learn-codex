...
> **这一页不是「读之前必须先看」。**
>
> 十五章里的每一个词，都在它第一次出现的地方就地解释清楚。
> 你不需要先学会任何东西才能开始 —— 直接去 [s01](s01_agent_loop/README.zh.md)。
>
> 这一页只是一张**查词表**：读到一半卡住了，翻回来查一下，然后接着读。
> 你也可以完全不看它。
...
> 每一章的代码都是逐行标注的，所以**你不需要先学 Python**。
> 下面这些只是给你一个印象，卡住的时候回来对一下。
...
`while True` 在 s01 里出现，而且那里逐行标注了它在干什么。你不必记住它。
...
……长得几乎一样。但要小心一个误解：**JSON 不是「Python 的东西」。**
它是从 JavaScript 那边来的，跟 Python 没有血缘关系，只是两边碰巧都用了大括号加冒号这种写法。

这件事之所以要说清楚，是因为你后面会看到一个 Rust 写的程序和一个 Python 写的程序用 JSON 对话
（[s13](s13_mcp/README.zh.md)）。它们能对上话，正是因为 **JSON 不属于任何一门语言**。
...
| token | 模型数东西的单位。英文大约半个到一个单词；**中文大约一个 token 一到两个字**，所以同样一段中文比英文更费 |
...
什么时候想开始都行：[s01: 一个循环 + 一个 shell](s01_agent_loop/README.zh.md)。
