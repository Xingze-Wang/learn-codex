...
> **没写过代码？那也直接从 [s01](s01_agent_loop/) 开始。** 这里没有任何东西需要你先学会 ——
> 每个词都在它第一次出现的地方就地解释，每段代码都是逐行标注的。
> 卡住了有一张[查词表](PRIMER-zh.md)，但没有任何一章需要它。
...
```
问题          一件会坏掉的事，用【你的世界】描述 —— 不是用上一章描述
（那个概念）   你需要知道的东西，就在需要它的地方讲，当场讲清
解决方案      一张图，加一张「信号 -> 含义 -> 动作」的表
工作原理      第 1 步…第 N 步，每步几行逐行标注的代码，加一句「为什么这样」
这一章改变了什么   这一章之前 / 之后
试一下        确切的命令，以及跑起来该看哪里
接下来        这一章留下的痛点，也就是下一章
```

**任何一章都可以直接翻开就读。** 每一章讲自己的问题时都不假设你读过上一章，
每个 `code.py` 也都能单独跑。按顺序读能看到每个机制**为什么会出现**，但跳着读不会坏。
...
任何地方卡住了，[查词表](PRIMER-zh.md) 里都有 —— 不过每一章也都会就地解释自己用到的词。
