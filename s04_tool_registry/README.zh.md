...
你的 agent 现在只会一件事：跑 shell 命令。

你想让它也能改文件、也能列个计划、也能查你们公司的工单系统。最直觉的做法是在代码里
再加一个 `if`，再加一个 `elif`，一路排下去。

这条路撑不住。不是因为 `if` 排得太长，而是因为下面这三件事**每一轮都可能不一样**：
...
|  | 这一章之前 | 这一章之后 |
|---|---|---|
| 加一个工具 | 循环里再加一个 `elif` | 一句 `registry.register(...)` |
| 工具清单 | 写死的 | 按模型 + 配置每轮组装 |
| 调用形状 | 只有 JSON | JSON 函数工具 **和** 自由格式文法工具 |
| 工具失败时 | 抛异常，会话结束 | 变成一段文字，模型读了自己改 |
| 工具输出 | 无限长 | 按 token 预算留头留尾 |
