...
让 agent 跑一条命令，最直觉的做法是：**启动它，等它结束，把输出拿走。**
...
|  | 这一章之前 | 这一章之后 |
|---|---|---|
| 跑一条命令 | `subprocess.run`，等它退出 | 一个 PTY 会话 |
| 永不退出的命令 | 把 agent 永久挂死 | 提前返回一个 session id |
| 跟正在跑的程序对话 | 做不到 | `write_stdin` |
| `cd build` | 下一次调用就丢了 | 在会话存活期间一直有效 |
| 杀掉它 | 子进程继续跑 | 整个进程组一起走 |
