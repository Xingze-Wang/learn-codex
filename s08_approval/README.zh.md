...
agent 想干一件被拦下来的事：装个依赖、写 `~/.npmrc`、跑一次需要联网的构建。

**这次拦截可能是对的，也可能是错的。** 而且只有你知道是哪一种。
...
而且「问一句」这件事本身就不容易：**如果跑这一轮的是一个普通函数，它根本问不出口。**
函数只能 return 一次，没法「问一句、等你答、然后接着跑」（这就是 [s02](../s02_protocol/) 要解决的）。
