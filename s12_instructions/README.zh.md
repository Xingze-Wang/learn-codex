...
|  | 这一章之前 | 这一章之后 |
|---|---|---|
| 提示词 | 一个硬编码字符串 | 四个来源，走三条通道 |
| 项目规矩 | 每次对话都要重说一遍 | `AGENTS.md`，自动发现 |
| monorepo | 全项目一套规矩 | 从根到 cwd，最近的文件赢 |
| 一份两千 token 的手册 | 要么每次请求都带，要么干脆没有 | 只挂一行描述，正文用到时再取 |
