...
你们团队有条规矩：**谁都不许直接 push，推送走 CI。**

agent 不知道这条规矩。而决定 agent 行为的那些代码，是写这个 agent 的人写的 —— 不是你。
...
一份「按命令前缀匹配」的规则文件能表达第一条（[s09](../s09_exec_policy/) 就是这个），
但表达不了第二、三条 —— 它只会比对命令的开头，没法跑任意逻辑。
...
|  | 这一章之前 | 这一章之后 |
|---|---|---|
| 改 harness 的策略 | fork 整个项目 | 写一个程序，登记进 `hooks.json` |
| 拦下一次工具调用 | 轮不到你 | `PreToolUse` 返回 `deny` |
| 注入你自己的上下文 | 轮不到你 | `additionalContext`，且有上限 |
| hook 崩了或挂了 | — | 一条警告；会话照常继续 |
